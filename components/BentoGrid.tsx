      ...
      <div>
        ...
        {tags && (
          <div className="flex flex-wrap gap-2 mb-6">
            {tags.map((tag: string) => (
              <span
                key={tag}
                className="text-[10px] font-black uppercase tracking-wider px-3 py-1 bg-white/5 rounded-full text-slate-300 border border-white/5"
              >
                {tag}
              </span>
            ))}
        ...
        {img && (
          <div className="mt-4 rounded-2xl overflow-hidden border border-white/10">
            <img
              src={typeof img === "string" ? img : img.props?.src}
              alt={title}
    ...
    <section id="expertise" className="py-32 px-6 max-w-7xl mx-auto">
      <div className="flex flex-col md:flex-row justify-between items-end mb-20 gap-8">
        ...
        >
          ...
          <h2 className="text-5xl md:text-7xl font-black tracking-tighter leading-[0.9] text-white">
            Architecting <br />
            <span className="text-slate-500">Scalable Systems.</span>
          </h2>
        </motion.div>
        <motion.p
          initial={{ opacity: 0 }}
        ...
        >
          Specializing in the intersection of high-end frontend engineering and
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6 auto-rows-auto md:auto-rows-[340px]">
        <BentoCard
          ...
          description="Developing blazing-fast, SEO-optimized React & Next.js applications that convert visitors into customers. From pixel-perfect UI with Tailwind to robust PostgreSQL architectures that scale with your business."
          tags={["React", "Next.js", "TypeScript", "Tailwind", "PostgreSQL"]}
          icon={
            <svg
              className="w-7 h-7 text-indigo-400 group-hover:text-white"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
        ...
        <BentoCard
          ...
          icon={
            <svg
              className="w-7 h-7 text-purple-400 group-hover:text-white"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
              <path d="M13 10V3L4 14h7v7l9-11h-7z" strokeWidth={2.5} />
            </svg>
          }
        />
        <BentoCard
          ...
          icon={
            <svg
              className="w-7 h-7 text-pink-400 group-hover:text-white"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
